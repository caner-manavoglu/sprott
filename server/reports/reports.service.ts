...
import { TODAY, idField } from '../common/fields.ts';
...
export class ReportsService {
  ...
  async index(req: AuthRequest) {
    const user = current(req), projectIds = await this.workspace.projectIds(user), { scope, groups, userIds } = await this.workspace.visibleUsers(user);
    ...
    const rows: { id: number; name: string; surname: string; title: string; assigned: number; completed: number; bugs: number; overdue: number }[] = (await sql(this.prisma, `
      WITH final AS (
  ...
  async detail(req: AuthRequest, rawId: string) {
    const viewer = current(req), id = idField(rawId), { userIds } = await this.workspace.visibleUsers(viewer);
    if (userIds && !userIds.includes(id)) throw new ForbiddenException('Bu personelin raporunu görme yetkiniz yok.');
    const person = (await sql(this.prisma, `SELECT id,name,surname,title,("avatarContent" IS NOT NULL) AS "hasAvatar" FROM users WHERE id=$1 AND role='user'`, [id])).rows[0];
    ...
    const projectIds = await this.workspace.projectIds(viewer);
    const rows: { projectId: number; projectName: string; assigned: number; completed: number; bugs: number; overdue: number }[] = projectIds.length ? (await sql(this.prisma, `
      WITH final AS (
      ...
      ORDER BY completed DESC, p.name`, [projectIds, id])).rows : [];
