...
import { liveMiddleware } from './common/live.middleware.ts';
...
export async function createApp() {
  ...
  app.use('/api', express.json(), sessionMiddleware(app.get(WorkspaceService)), liveMiddleware());
  const config = new DocumentBuilder()
    .setTitle('Sprott REST API')
    .setDescription('Önce POST /api/login üzerinden giriş yapın. Tarayıcı httpOnly oturum çerezini kendisi taşır; betikler için yanıttaki "token" değerini sağ üstteki Authorize düğmesine yapıştırın. Oturumsuz istekler 401 döner. Yönetici işlemleri açıklamalarda belirtilmiştir.')
    .setVersion('0.1.0')
