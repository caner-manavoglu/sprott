...
test('MCP E2E: own tasks only, strict tools, workflow, stale writes, permissions and revocation', async () => {
  ...
  try {
    ...
    await store.db.query(`UPDATE users SET permissions='{"task.view":true,"task.update":true}' WHERE id=2`);
    ...
    const task = async (name: string, owner: number | null) => (await store.db.query('INSERT INTO tasks(title,description,"columnId","createdBy","assigneeId") VALUES($1,$1,1,1,$2) RETURNING id', [name, owner])).rows[0].id;
    const own = await task('Own task', 2), other = await task('Private task', 1), unassigned = await task('Unassigned', null);
    await store.db.query('INSERT INTO workflow_transitions VALUES(1,1,2),(1,2,3)');
    ...
    assert.equal((await store.db.query('SELECT "columnId" FROM tasks WHERE id=$1',[own])).rows[0].columnId, 2);
    assert.equal((await store.db.query('SELECT count(*)::int AS count FROM activity_log WHERE "taskId"=$1',[own])).rows[0].count, 1);
    await call('transition_task', {taskId: own, columnId: 3, expectedColumnId: 1}, true);
    await store.db.query('UPDATE projects SET "completedAt"=now() WHERE id=1');
    await call('transition_task', {taskId: own, columnId: 3, expectedColumnId: 2}, true);
    await store.db.query('UPDATE projects SET "completedAt"=NULL WHERE id=1');
    await call('transition_task', {taskId: own, columnId: 3, expectedColumnId: 2});
    assert.equal((await store.db.query('SELECT "columnId" FROM tasks WHERE id=$1',[own])).rows[0].columnId, 3);
    await store.db.query('UPDATE tasks SET "columnId"=2 WHERE id=$1', [own]);
    await store.db.query(`UPDATE users SET permissions='{"task.view":true}' WHERE id=2`);
    await call('transition_task', {taskId: own, columnId: 3, expectedColumnId: 2}, true);
    await store.db.query(`UPDATE users SET role='admin' WHERE id=2`);
    ...
    await store.db.query('UPDATE tasks SET "assigneeId"=1 WHERE id=$1', [own]);
    await call('get_task', {taskId: own}, true);
    await store.db.query('UPDATE mcp_tokens SET expires=0 WHERE id=$1', [key.id]);
